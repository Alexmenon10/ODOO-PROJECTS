...
#     res_partner = fields.Many2one('res.partner', string="Requesting Branch",default=lambda self: self.env.user.partner_id.id)
    ...
    state = fields.Selection(
            [('draft', 'Draft'),
             ...
             ('done', 'Done'),
             ('cancel', 'Cancel'),
             ('reject', 'Rejected')], string='State', readonly=True, default='draft', tracking='onchange')
    procurement_group_id = fields.Many2one('procurement.group', 'Procurement Group', copy=False)
    requesting_person_name = fields.Char('Requesting Person Name', size=30)
    indent_creator_name = fields.Many2one('res.users',string = 'Indent Creator Name', size=30)
    store_type = fields.Selection([('transfer', 'Internal Transfer'),('purchase','Purchase')], string='Indent Type')
    purchase_count = fields.Integer(compute='_get_purchase_count',string="Purchase Count")


    def _get_purchase_count(self):
        self.purchase_count = len(self.env['purchase.order'].search([('purchase_indent_id','=',self.id)]))
    ...
    def get_purchase_orders(self):
        ...
        return {
            ...
            'view_mode': 'list,form',
            'views': [[view.id, 'list'],[form_view.id,'form']],
            'domain': [('purchase_indent_id', '=', self.id)]
        }

    def purchase_indent_create(self):
        purchase_orders1 = self.env['purchase.order']
        purchase_dict = {}
        prod_qt = {}
        if self.store_type == 'purchase':
            for each in self.product_lines:
                prod_qt[each.product_id.id] = each.product_uom_qty
                for vendors in each.product_id.seller_ids:
                    if purchase_dict.get(vendors.name.id):
                        value = purchase_dict.get(vendors.name.id)
                        value.append(each.product_id)
                    else:
                        purchase_dict[vendors.name.id] = [each.product_id]
            ...
            for key,value in purchase_dict.items():
                ...
                for each in value:
                    values = {
                        'price_unit': each.list_price or 0.0,
                        'product_id': each.id,
                        'product_uom': each.uom_id.id,
                        'product_qty': prod_qt.get(each.id),
                        'name': each.name,
                        ...
                        if line.location_dest_id:
                            ...
                            if line.product_id.type == 'consu':
                                move_line = {
                                    ...
                                    'date_deadline': self.required_date,
                                    'invoice_state': "none",
                                    'origin': name,
                                    'purchase_indent_id': self.id
                                }
                                ...
                                if tot_qty >= line.product_uom_qty:
                                    ...
                                    if tot_qty:
                                        raise ValidationError((" No sufficient stock for product ' %s ' in '%s'.  "
                            ...
                            raise ValidationError((" Destination Location is not set properly for' %s '. "
                                                      "So Plese cancel this indent and create a new one please.")
                                                     % line.product_id.name)
                    else:
                        raise ValidationError(("Source Location is not set properly for ' %s '.  "
                                                  "Please go and set Source Location.")
                                                 % line.product_id.name)
                else:
                    raise ValidationError("This product is a service type product.")
        else:
            raise ValidationError('You cannot Transfer a order without any product line.')
...
#         print("Time",  my_dist["duration"]["text"])
    ...
    original_product_id = fields.Many2one('product.product', string='Product to be Manufactured', readonly=True,
                                          states={'draft': [('readonly', False)], 'waiting_approval': [('readonly', False)]})
    product_uom_qty = fields.Float(string='Quantity Required', digits=dp.get_precision('Product UoS'), required=True,
                                   readonly=True, states={'draft': [('readonly', False)],
...
#                                                'waiting_approval': [('readonly', False)]})
    ...
    unknown_product = fields.Char(string='Product')
    product_category = fields.Selection(
        [('admin', 'Admin Request'),
         ('em', 'EM Request')], string='Product Category')
...
#         print(self.res_partner,"==============================opopopopopo")
...
#         self.location_dest_id = location_id.id
    ...
    def current_location(self):
        l_list = []
        if self.current_location_ids:
            vals = {'current_location_id': False,
                    'indent_line_id': False,
                    'quantity': False}
            self.current_location_ids.update(vals)
        if self.product_id:
            ...
            if c_location:
                for p in c_location:
                    ...
                    if p.location_id.usage in ['internal','customer']:
                        vals = {'current_location_id': p.location_id.id,
                                        'indent_line_id':self.id,
                                        'quantity':p.quantity}
                        self.current_location_ids = [(0,0,vals)]
            else:
...
class PurchaseOrder(models.Model):
    ...
    purchase_indent_id = fields.Many2one('purchase.indent', string='Purchase Indent')

# class SubCateExpensive(models.Model):
#     _name = 'subcate.expense.category'
#
#     name = fields.Char(string="Sub Category")
#     category_id = fields.Many2one('product.product', string="Category")
...
class HrExpense(models.Model):
    ...
    # subcategory_id = fields.Many2one('subcate.expense.category', string="Sub Category", tracking=True, domain="[('category_id', '=', product_id)]")
